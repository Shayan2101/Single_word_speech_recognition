...
from keras._tf_keras.keras.layers import Dense, Dropout, Flatten, Conv2D, MaxPooling2D, LSTM,RepeatVector
...
tensorboard_callback = TensorBoard(log_dir='./logs_dense', histogram_freq=1)

model.fit(X_train, y_train_hot, epochs=epochs, validation_data=(X_test, y_test_hot), callbacks=[tensorboard_callback])

# Model with Conv2D and LSTM
model = Sequential()
model.add(Conv2D(32, (3, 3), input_shape=(buckets, max_len, channels), activation="relu"))
